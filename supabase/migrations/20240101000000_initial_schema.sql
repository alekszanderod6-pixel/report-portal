...
create policy "Admin view all entries" on public.report_entries for select using (exists (select 1 from public.reports where id = report_id and user_id = auth.uid()));
